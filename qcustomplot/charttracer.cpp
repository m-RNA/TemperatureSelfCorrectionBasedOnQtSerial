 ...
 **********************************************
 */
ChartTracer::ChartTracer(QCustomPlot *_plot, QCPGraph *_graph, TracerType _type) : plot(_plot),
                                                                                   graph(_graph),
                                                                                   type(_type),
                                                                                   visible(false)
...
void ChartTracer::setLabelFont(QFont font)
{
    label->setFont(font);
}

void ChartTracer::setPen(const QPen &pen)
{
    tracer->setPen(pen);
    arrow->setPen(pen);
}

void ChartTracer::setBrush(const QBrush &brush)
{
    tracer->setBrush(brush);
}

void ChartTracer::setLabelPen(const QPen &pen)
{
    label->setPen(pen);
}

void ChartTracer::setText(const QString &text)
{
    label->setText(text);
}

void ChartTracer::setVisible(bool visible)
{
...
void ChartTracer::updatePosition(double xValue, double yValue)
{
    if (!visible)
    {
        setVisible(true);
